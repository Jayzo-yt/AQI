...
TARGET_DATE = os.environ.get('TARGET_DATE', '2020-01-15')   # read from env, default to 2020-01-15

# India bounding box (approximate, slightly generous to ensure full coverage)
LAT_MIN, LAT_MAX = 8.0, 37.0
LON_MIN, LON_MAX = 68.0, 97.5
GRID_STEP = float(os.environ.get('GRID_STEP', '0.25'))  # degrees -> ~6,800 points over this bounding box if 0.25
...
OUTPUT_DIR = r"C:\Users\Jayyanth\Desktop\ISRO\data\processed\output\grid"
RES_SUFFIX = "fast" if GRID_STEP >= 1.0 else "high"
OUTPUT_CSV = os.path.join(OUTPUT_DIR, f"grid_extracted_{TARGET_DATE}_{RES_SUFFIX}.csv")
PROGRESS_LOG = os.path.join(OUTPUT_DIR, f"grid_progress_{TARGET_DATE}_{RES_SUFFIX}.txt")
