...
INPUT_CSV = r"C:\Users\Jayyanth\Desktop\ISRO\data\processed\output\final_dataset_v3.csv"
...
trained_models['rf_random'] = rf_random
...
trained_models['rf_station'] = rf_station
...
trained_models['xgb_random'] = xgb_random
...
trained_models['xgb_station'] = xgb_station
...
# =====================================================================
#
# SELECTION CRITERION: lowest RMSE on the STATION-HELD-OUT test set,
# not the random-split test set. This is a deliberate choice, not a
# default — Objective 1's actual goal is a spatial AQI map across
# India, meaning the model needs to generalize to NEW locations, not
# just predict well at stations it already trained on. Random-split
# performance can look good purely from memorizing per-station
# baselines, which would NOT transfer to an unmonitored location.
#
# If you disagree with this criterion (e.g. you only care about
# filling gaps between EXISTING stations, not truly new locations),
# change CANDIDATE_KEYS below to the *_random keys instead.

CANDIDATE_KEYS = ['rf_station', 'xgb_station']  # only compare on the harder, honest test

best_key = min(CANDIDATE_KEYS, key=lambda k: results[k]['rmse'])
best_model = trained_models[best_key]
best_metrics = results[best_key]

print("\n" + "=" * 70)
print("BEST MODEL SELECTION (by station-held-out test RMSE)")
print("=" * 70)
for k in CANDIDATE_KEYS:
    marker = "  <-- SELECTED" if k == best_key else ""
    print(f"  {k:<15} RMSE={results[k]['rmse']:.2f}  MAE={results[k]['mae']:.2f}  "
          f"R={results[k]['r']:.3f}{marker}")

os.makedirs(MODEL_DIR, exist_ok=True)
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
model_filename = f"best_pm25_model_{best_key}_{timestamp}.joblib"
model_path = os.path.join(MODEL_DIR, model_filename)

joblib.dump(best_model, model_path)
print(f"\nSaved best model to: {model_path}")

# Save a metadata file alongside it — without this, a saved .joblib file
# is just a black box six months from now. Always pair a saved model
# with what features it expects, in what order, and how it scored.
metadata = {
    "model_key": best_key,
    "model_type": "RandomForestRegressor" if "rf" in best_key else "XGBRegressor",
    "split_strategy": "station_holdout",
    "selection_criterion": "lowest RMSE on station-held-out test set",
    "feature_columns_in_order": FEATURE_COLS,
    "target_column": TARGET_COL,
    "test_metrics": best_metrics,
    "all_candidate_metrics": {k: results[k] for k in CANDIDATE_KEYS},
    "all_model_metrics_full": {k: v for k, v in results.items()},
    "trained_on_rows": len(df_rf) if "rf" in best_key else len(df),
    "saved_at": timestamp,
    "input_csv": INPUT_CSV,
}

metadata_path = os.path.join(MODEL_DIR, f"best_pm25_model_{best_key}_{timestamp}_metadata.json")
...
print(f"    # feature columns must be passed in this exact order: {FEATURE_COLS}")